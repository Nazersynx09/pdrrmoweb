...
export async function getIssuanceId(request: Request) {
...
export async function POST(request: NextRequest) {
  ...
  const { title, type, number, date_issued, file_url, description, is_active } = body;
  if (!title) return NextResponse.json({ success: false, error: 'Title required' }, { status: 400 });

  const { data, error } = await client.from('issuances').insert([{
    title,
...
export async function PUT(request: NextRequest) {
  ...
  const { data, error } = await client.from('issuances').update(updates).eq('id', id).select().single();
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 });
  return NextResponse.json({ success: true, data });
}
...
export async function DELETE(request: NextRequest) {
  ...
  const { error } = await client.from('issuances').delete().eq('id', id);
  if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 });
